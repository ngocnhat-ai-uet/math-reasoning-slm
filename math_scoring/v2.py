...
DEGREE_RE = re.compile(r"(?:\^\s*\{?\s*\\circ\s*\}?|°)\s*$")
...
def _normalize_latex_surface(text: str) -> str:
    ...
    s = re.sub(r"_\{\s*([A-Za-z0-9+\-]+)\s*\}", r"_\1", s)
...
def _choice_match(gt: str, pred: str) -> bool:
    ...
    pred_clean = _normalize_for_literal_compare(pred).upper()

    if "," in gt_clean:
        gt_choices = gt_clean.replace(",", "")
        if gt_choices and set(gt_choices).issubset(CHOICE_SET):
            return pred_clean.replace(",", "") == gt_choices
...
def _numeric_candidate_forms(text: str) -> set[float]:
    """Return numeric interpretations, adding ratio form only for explicit percents."""
    s = _strip_unit_suffix(to_text(text)).strip()
...
def _numeric_match(gt: str, pred: str) -> bool:
    """
    Compare numeric values with strict percentage-aware tolerance.

    Explicit percentages match both their displayed value and ratio value, so
    70%, 70, and 0.7 can match. Plain 70 and 0.7 do not match without a percent
    marker on at least one side.
    """
    gt_forms = _numeric_candidate_forms(gt)
    pred_forms = _numeric_candidate_forms(pred)
    if not gt_forms or not pred_forms:
        return False

    for g in gt_forms:
        for p in pred_forms:
            if _numbers_close(p, g):
                return True
    return False
